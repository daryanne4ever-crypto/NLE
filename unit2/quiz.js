...
a.addEventListener('click',e=>{const b=e.target.closest('[data-i]');if(!b)return;const ok=Number(b.dataset.i)===qs[i].c;if(ok){addUnitXP('unit2',15,`Unit1 Quiz Q${i+1}`);f.textContent=`Correto! +15 XP. ${qs[i].tip}`;f.className='feedback good';}else{f.textContent=`Resposta correta: ${qs[i].a[qs[i].c]}. ${qs[i].tip}`;f.className='feedback bad';}});
